...
void YawInitialPlanner::searchPathOfYaw(const vector<Eigen::Vector3d> &pos,
  ...
  for (int i = 0; i < num_layers; ++i) {
    ...
    /*
      整个for循环完成了图搜索和每条边的价值计算


      vert的形成：
      （IF部分）
      对于起点和终点，直接初始化，不需要进行yaw搜索
      （else部分）
      对于中间层，即每一个路径点对应的层，p1,p2,p3,p4...
      对于p1层，首先采样这个层中偏航角序列，放入到yaw_samples，将采样的每一个偏航角形成的vert（定点），加入到yaw_graph和layer


      计算当前layer中每一个vert与上一层last_layer每一个vert之间的价值

      注：起点和终点各有一个vert，中间的每一个层都有n个vert，每一层的每一个偏航角都形成一个vert
    */
