...
//计算相机 FOV 的四个边界平面法向量
  ...
  // FOV vertices in body frame, for FOV visualization计算 FOV 顶点（用于可视化）
  ...
  /*
  计算 FOV 在机体坐标系下的四个角点：
  计算 水平/垂直偏移：hor = vis_dist_ * tan(left_angle_)，vert = vis_dist_ * tan(top_angle_)
  vis_dist_ 决定了 FOV 的深度。
  */
...
//计算 FOV 包围盒
/*
FOV是由相机的位置、视场角和最大感知距离所定义的一个区域。
包围盒则是用来描述该区域的最小矩形或立方体，通常用于后续的计算或可视化。
*/
