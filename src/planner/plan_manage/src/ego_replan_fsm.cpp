...
*/
  ...
  void EGOReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg)
  {
    ...
    if (success)
    {
      ...
      /*** FSM ***/
      /*
      如果当前状态是等待目标，那么就切换到生成新轨迹的状态，对应无人机第一次执行轨迹或者已经执行完轨迹再给出新的目标点，那么就切换到生成新轨迹的状态
      如果当前状态是执行轨迹，那么就切换到重新规划轨迹的状态，对应在无人机执行当前任务时，又给出新的目标点，那么就切换到重新规划轨迹的状态
      */
      if (exec_state_ == WAIT_TARGET)
        changeFSMExecState(GEN_NEW_TRAJ, "TRIG");
      else if (exec_state_ == EXEC_TRAJ)
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    /*
      ...
      比如进入gen_new_traj，会进入callReboundReplan进行第一次重规划然后发布b样条，第一次重规划的初始加速度为0，
      然后进入exec_traj，获取当前无人机位置，
      判断当前位置和目标点距离，如果距离小于一定值，则不再重规划，因为到达目标点了，
      如果距离比较远，则进入replan_traj，从当前位置进行局部规划，planFromCurrentTraj()是进行局部轨迹的函数，
      函数本身会在规划后发布b样条
...
//碰撞检测的回调函数
  ...
  {
    LocalTrajData *info = &planner_manager_->local_data_;    //获得无人机的局部轨迹信息
    auto map = planner_manager_->grid_map_;                  //获取地图信息
    ...
    double t_cur = (ros::Time::now() - info->start_time_).toSec();  //获取此段轨迹运行的当前时间
    double t_2_3 = info->duration_ * 2 / 3;                        //获取此段轨迹运行需要的总时间的2/3
