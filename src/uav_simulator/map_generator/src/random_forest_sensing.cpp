...
void RandomMapGenerate() {
  ...
  //根据前面的参数限制，随机生成障碍物的x，y的位置，宽，高，球的半径，旋转角度，z轴深度
  ...
  kdtreeLocalMap.setInputCloud(cloudMap.makeShared());   //将点云地图传递给KdTree对象kdtreeLocalMap，以便进行点云处理和查询操作。
...
void pubSensedPoints() {
  ...
  /* ---------- only publish points around current position ---------- 发布周围的地图*/  
...
int main(int argc, char** argv) {
  ...
  n.param("init_state_y", _init_y, 0.0); //初始化飞机位置

  n.param("map/x_size", _x_size, 50.0);
  n.param("map/y_size", _y_size, 50.0);
  n.param("map/z_size", _z_size, 5.0);      //地图大小
  n.param("map/obs_num", _obs_num, 30);            //障碍物数量
  n.param("map/resolution", _resolution, 0.1);   //分辨率
  n.param("map/circle_num", circle_num_, 30);    //  圆的数量

  n.param("ObstacleShape/lower_rad", _w_l, 0.3);
  n.param("ObstacleShape/upper_rad", _w_h, 0.8);       //障碍物宽度
  n.param("ObstacleShape/lower_hei", _h_l, 3.0);
  n.param("ObstacleShape/upper_hei", _h_h, 7.0);       //障碍物高度上下限

  n.param("ObstacleShape/radius_l", radius_l_, 7.0);        //障碍物半径上下限
  n.param("ObstacleShape/radius_h", radius_h_, 7.0);
  n.param("ObstacleShape/z_l", z_l_, 7.0);
  n.param("ObstacleShape/z_h", z_h_, 7.0);                  //
  n.param("ObstacleShape/theta", theta_, 7.0);               //障碍物旋转角度

  n.param("sensing/radius", _sensing_range, 10.0);        //感知范围
  ...
  _y_l = -_y_size / 2.0;
  _y_h = +_y_size / 2.0;    //定义边界

  _obs_num = min(_obs_num, (int)_x_size * 10);  //最小边界
  _z_limit = _z_size;
